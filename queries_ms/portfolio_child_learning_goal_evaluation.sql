-- residual_count
-- portfolio_child_learning_goal_evaluation
...
INNER JOIN {ARCHIVED_MS_CLASSOPS_DB}.portfolio_child_learning_goal_evaluation clg ON clg.fk_portfolio = pat.id
WHERE ch.id IN(
...
INNER JOIN {SOURCE_MS_CLASSOPS_DB}.portfolio_child_learning_goal_evaluation clg ON clg.fk_portfolio = pat.id
WHERE ch.id NOT IN(
SELECT darch.fk_child FROM {ARCHIVED_SN2_DB}.dar_child_archival_delete_unique_map darch
WHERE ch.id = darch.fk_child
)
